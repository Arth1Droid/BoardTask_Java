...
public class BoardColumnDAO {
    ...
    public BoardColunmEntity insert(final BoardColunmEntity entity) throws SQLException{
        var sql =  "INSERT INTO BOARDS_COLUNMS (name, `order`, kind, board_id) VALUES (?, ?, ?, ?);";
    ...
    public List<BoardColunmEntity> findByBoardId(final Long boardId) throws SQLException{
    ...
    public List<BoardColumnDTO> findByBoardIdWithDetails(final Long boardId) throws SQLException{
        ...
        var sql = "SELECT bc.name, bc.kind, c.id, c.title, c.description FROM BOARDS_COLUMNS bc LEFT JOIN CARDS c ON c.board_colum_id = bc.id WHERE bc.id = ?";
        try(var statement = connection.prepareStatement(sql)){
            statement.setLong(1, boardId);
            statement.executeQuery();
            var resultSet = statement.getResultSet();
